...
from models.loss import CharbonnierLoss, VGGLoss, SSIMLoss, ShiftFreeLoss
...
class VideoBaseModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            elif loss_type == 'shift_free':
                self.cri_pix = ShiftFreeLoss().to(self.device)
                print('using shift_free loss')
            else:
    ...
    def print_network(self):
        ...
        if self.rank <= 0:
            ...
            if self.opt['logger'].get('print_network', False):
                logger.info(s)
