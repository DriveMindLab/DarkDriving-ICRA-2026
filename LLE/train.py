...
def main():
    ...
    if rank <= 0:
        if resume_state is None:
            ...
            if opt.get('logger', {}).get('save_training_state', False):
                util.mkdir(opt['path']['training_state'])
            if opt.get('train', {}).get('save_val_images', False):
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
                ...
                if opt['model'] in ['sr', 'srgan'] and rank <= 0:
                    ...
                    if opt['dist']:
                        ...
                        for idx in range(rank, len(val_set), world_size):
                            ...
                            rlt_img2 = util.tensor2img(visuals['rlt2'])
                            gt_img = util.tensor2img(visuals['GT'])
            ...
            save_checkpoint_freq = opt['logger'].get('save_checkpoint_freq', 0)
            if save_checkpoint_freq and current_step % save_checkpoint_freq == 0:
                if rank <= 0:
                    logger.info('Saving model checkpoint.')
                    model.save(current_step)
                    if opt['logger'].get('save_training_state', False):
